...
<style>
:root{
  --sf-navy:#032d60;--sf-blue:#0176d3;--sf-bg:#f3f3f3;--sf-white:#fff;
  --sf-border:#dddbda;--sf-text:#181818;--sf-muted:#706e6b;--sf-label:#514f4d;
  --sardine-ink:#1a1033;
  --risk-low:#2e844a;--risk-low-bg:#eaf5ea;
  --risk-med:#dd7a01;--risk-med-bg:#fdf3e3;
  --risk-high:#ba0517;--risk-high-bg:#fde8e8;
...
.last-refresh{font-size:10px;color:#88c3fb;font-family:var(--font-mono)}
...
.acct-tab{padding:8px 18px;background:#e8e8e8;cursor:pointer;font-size:12px;font-weight:500;color:var(--sf-muted);display:flex;align-items:center;gap:7px;white-space:nowrap}
.acct-tab.active{background:var(--sf-white);color:var(--sf-blue);font-weight:600}
.tab-risk{font-size:10px;font-weight:600;padding:1px 6px;border-radius:100px;margin-left:2px}
.tab-risk.low{background:var(--risk-low-bg);color:var(--risk-low)}
.tab-risk.medium{background:var(--risk-med-bg);color:var(--risk-med)}
.tab-risk.high,.tab-risk.very_high{background:var(--risk-high-bg);color:var(--risk-high)}
...
/* ── Highlights bar — with risk-aware cell backgrounds ── */
.hl-bar{display:flex;border-bottom:1px solid var(--sf-border);overflow-x:auto}
.hl-item{padding:10px 16px;border-right:1px solid var(--sf-border);min-width:100px;flex-shrink:0;transition:background .4s}
.hl-item.risk-low{background:var(--risk-low-bg)}
.hl-item.risk-med{background:var(--risk-med-bg)}
.hl-item.risk-high{background:var(--risk-high-bg)}
.hl-label{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.6px;color:var(--sf-muted);margin-bottom:3px}
.hl-item.risk-low .hl-label{color:#1a5c30;opacity:.8}
.hl-item.risk-med .hl-label{color:#7a4400;opacity:.8}
.hl-item.risk-high .hl-label{color:#7a0010;opacity:.8}
.hl-value{font-size:13px;font-weight:700;color:var(--sf-text)}
.hl-item.risk-low .hl-value{color:#1a5c30}
.hl-item.risk-med .hl-value{color:#7a4400}
.hl-item.risk-high .hl-value{color:#7a0010}

/* ── Record body ── */
.rec-body{display:grid;grid-template-columns:1fr 400px;min-height:480px}
.rec-left{padding:20px;border-right:1px solid var(--sf-border)}
.rec-right{background:#f8f8f8;display:flex;flex-direction:column}
...
.field-value.awaiting{color:#bbb;font-style:italic}
...
.sp-sub{font-size:10px;color:#a0a0c0;margin-left:auto;font-family:var(--font-mono)}

/* ── Synced-ago ticker ── */
.sync-ticker{font-size:10px;color:#a0a0c0;font-family:var(--font-mono)}
.sync-ticker.stale{color:#f87171}

.sp-body{padding:12px 14px;display:flex;flex-direction:column;gap:10px;flex:1;overflow-y:auto}

/* ── Risk hero ── */
.risk-hero{border:1px solid var(--sf-border);border-radius:6px;padding:12px 14px;background:var(--sf-white);display:flex;align-items:center;justify-content:space-between;transition:background .4s,border-color .4s}
.risk-hero.low{background:var(--risk-low-bg);border-color:#b8ddb8}
.risk-hero.medium{background:var(--risk-med-bg);border-color:#f5d8a0}
.risk-hero.high,.risk-hero.very_high{background:var(--risk-high-bg);border-color:#f5b8b8}
.risk-badge{font-size:13px;font-weight:700;padding:4px 14px;border-radius:100px}
.risk-badge.low{background:var(--risk-low-bg);color:var(--risk-low);border:1.5px solid #b8ddb8}
.risk-badge.medium{background:var(--risk-med-bg);color:var(--risk-med);border:1.5px solid #f5d8a0}
.risk-badge.high,.risk-badge.very_high{background:var(--risk-high-bg);color:var(--risk-high);border:1.5px solid #f5b8b8}
.risk-sub{font-size:10px;color:var(--sf-muted);margin-top:3px}

/* ── Signal groups ── */
.sig-group{border:1px solid var(--sf-border);border-radius:6px;background:var(--sf-white);overflow:hidden}
.sig-grp-hdr{padding:6px 12px;background:#f4f4f4;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:var(--sf-label);border-bottom:1px solid var(--sf-border);display:flex;align-items:center;gap:6px}
...
.ev-item{display:flex;align-items:center;gap:8px;padding:6px 12px;border-bottom:1px solid #f4f4f4;font-size:11px}
.ev-item:last-child{border-bottom:none}
.ev-icon{font-size:12px;flex-shrink:0}
.ev-time{font-family:var(--font-mono);font-size:10px;color:var(--sf-muted);white-space:nowrap;min-width:56px}
.ev-desc{color:var(--sf-text);line-height:1.4;flex:1}
.ev-tag{font-size:9px;font-weight:700;padding:2px 6px;border-radius:3px;white-space:nowrap;margin-left:auto}
...
/* ── Demo controls — same font/size as connector log ── */
.demo-lbl{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:#8888aa;display:flex;align-items:center;gap:6px;white-space:nowrap}
.demo-lbl::before{content:'';display:inline-block;width:6px;height:6px;border-radius:50%;background:#3dd68c;animation:pulse 2s ease-in-out infinite;flex-shrink:0}
.demo-btns{display:flex;flex-wrap:wrap;gap:5px;align-items:center}
.demo-btn{font-size:10px;font-weight:600;font-family:var(--font-mono);padding:2px 8px;border-radius:3px;border:1px solid;cursor:pointer;transition:opacity .15s;background:transparent}
.demo-btn:hover{opacity:.8}
.demo-btn.danger{color:#f87171;border-color:#f87171}
.demo-btn.warning{color:#fbbf24;border-color:#fbbf24}
.demo-btn.success{color:#3dd68c;border-color:#3dd68c}

/* ── Connector log bar (bottom) ── */
.conn-log{margin-top:14px;border:1px solid var(--sf-border);border-radius:6px;background:var(--sardine-ink);overflow:hidden}
.conn-log-hdr{padding:6px 12px;display:flex;align-items:center;gap:8px;border-bottom:1px solid rgba(255,255,255,.08)}
...
.log-line{font-family:var(--font-mono);font-size:10px;padding:2px 12px;display:flex;gap:8px;align-items:baseline}
.log-t{color:#5060a0;flex-shrink:0}.log-type{color:#88c3fb;font-weight:600;flex-shrink:0}.log-msg{color:#c0c0e0}
...
<div class="page">
  ...
  <div class="record">
    ...
    <!-- Highlights bar -->
    <div class="hl-bar">
      <div class="hl-item" id="hlRiskCell"><div class="hl-label">Sardine Risk</div><div class="hl-value" id="hlRisk">—</div></div>
      <div class="hl-item" id="hlAlertCell"><div class="hl-label">Alert Status</div><div class="hl-value" id="hlAlert">—</div></div>
      <div class="hl-item" id="hlCaseCell"><div class="hl-label">Case Status</div><div class="hl-value" id="hlCase">—</div></div>
      <div class="hl-item" id="hlKycCell"><div class="hl-label">KYC Status</div><div class="hl-value" id="hlKyc">—</div></div>
      <div class="hl-item" id="hlSanctionCell"><div class="hl-label">Sanction Hit</div><div class="hl-value" id="hlSanction">—</div></div>
    ...
    <div class="rec-body">
      ...
      <div class="rec-left">
        ...
        <div class="evlog">
          <div class="evlog-list" id="sardineEvents">
            <div class="ev-item"><div class="ev-desc awaiting">⏳ Awaiting events…</div></div>
          </div>
        </div>
      </div>
      ...
      <div class="rec-right">
        <div class="sardine-panel">
          <div class="sp-hdr">
            ...
            <div class="sp-sub"><span class="sync-ticker" id="syncTicker">Not yet synced</span></div>
          </div>
          ...
          <div class="sp-body">
            ...
            <div class="risk-hero" id="riskHero">
              <div>
                ...
                <div class="risk-sub">Overall Risk Level</div>
              </div>
            ...
            <!-- AML / Sanctions -->
            <div class="sig-group">
              <div class="sig-grp-hdr">🛡️ AML / Sanctions Screening</div>
            ...
            <div class="evlog">
              ...
              <div class="evlog-list" id="connEvents">
                <div class="ev-item"><div class="ev-desc awaiting">⏳ Awaiting events…</div></div>
              </div>
            </div>
          </div>
  ...
  <!-- Demo controls — full width, same style as connector log -->
  <div class="conn-log" style="margin-top:10px">
    <div class="conn-log-hdr">
      <div class="demo-lbl">Demo Controls</div>
      <div class="demo-btns" id="demoBtns" style="margin-left:12px"></div>
    </div>
  </div>
  ...
  <div class="conn-log">
    <div class="conn-log-hdr">
      <div class="dot"></div>
      <div class="conn-log-title">Connector Event Stream</div>
    </div>
    <div class="conn-log-body" id="connLogBody">
      <div class="log-line"><span class="log-msg">Connecting…</span></div>
    </div>
  </div>
</div>

<script>
let accounts = [], selectedId = null, connEvents = [], lastSyncedAt = null;

const fmt = v => (v !== null && v !== undefined && v !== '') ? v : null;
const fmtOrDash = v => fmt(v) ?? '—';
const fmtOrAwaiting = (el, v) => {
  const val = fmt(v);
  if (val) { el.textContent = val; el.classList.remove('awaiting'); }
  else      { el.textContent = 'Awaiting sync…'; el.classList.add('awaiting'); }
};
...
const hlClass = rk => rk==='low'?'risk-low':rk==='medium'?'risk-med':(rk==='high'||rk==='very_high')?'risk-high':'';
const hitHtml = v => v==='Yes'?'<span class="hit-yes">⛔ Yes</span>':v==='No'?'<span class="hit-no">✅ No</span>':'—';
...
const EVENT_META = {
  'alert.created':   {tag:'alert',    icon:'🔔'},
  'alert.updated':   {tag:'alert',    icon:'🔔'},
  'case.created':    {tag:'case',     icon:'📁'},
  'case.updated':    {tag:'case',     icon:'📁'},
  'document_verification.processed':{tag:'doc',icon:'🪪'},
  'document_verification.expired':  {tag:'doc',icon:'🪪'},
  'sanctions':       {tag:'sanctions',icon:'🛡️'},
};

// ── Live "X seconds ago" ticker ───────────────────────────────────────────
function startSyncTicker() {
  setInterval(() => {
    const el = document.getElementById('syncTicker');
    if (!lastSyncedAt) return;
    const secs = Math.floor((Date.now() - lastSyncedAt) / 1000);
    el.textContent = secs < 5 ? 'Just synced' : \`Synced \${secs}s ago\`;
    el.className = 'sync-ticker' + (secs > 30 ? ' stale' : '');
  }, 1000);
}
...
function renderTabs() {
  document.getElementById('tabs').innerHTML = accounts.map(a => {
    ...
    const riskLabel = rk ? rk.replace('_',' ') : '';
    return \`<div class="acct-tab\${a.Id===selectedId?' active':''}" onclick="select('\${a.Id}')">
      \${a.Name}
      \${riskLabel ? \`<span class="tab-risk \${rk}">\${riskLabel}</span>\` : ''}
    </div>\`;
  }).join('');
}
...
function renderAccount() {
  ...
  document.getElementById('metaIndustry').textContent = fmtOrDash(a.Industry);
  document.getElementById('metaOwner').textContent    = fmtOrDash(a.AccountOwner);
  document.getElementById('metaType').textContent     = fmtOrDash(a.Type);

  // Highlights bar — color-coded cells
  const rk = riskKey(a.Sardine_Risk_Level__c);
  const hc = hlClass(rk);
  document.getElementById('hlRisk').textContent    = fmtOrDash(a.Sardine_Risk_Level__c);
  document.getElementById('hlAlert').textContent   = fmtOrDash(a.Sardine_Alert_Status__c);
  document.getElementById('hlCase').textContent    = fmtOrDash(a.Sardine_Case_Status__c);
  document.getElementById('hlKyc').textContent     = fmtOrDash(a.Sardine_KYC_Status__c);
  document.getElementById('hlSanction').innerHTML  = a.Sardine_Sanction_Hit__c==='Yes'?'⛔ Yes':a.Sardine_Sanction_Hit__c==='No'?'✅ No':'—';
  ...
  // Color risk cell
  ['hlRiskCell','hlAlertCell','hlCaseCell'].forEach(id=>{
    const el=document.getElementById(id);
    el.className='hl-item '+(id==='hlRiskCell'&&hc?hc:'');
  });
  const sancCell=document.getElementById('hlSanctionCell');
  sancCell.className='hl-item '+(a.Sardine_Sanction_Hit__c==='Yes'?'risk-high':'');

  // Standard fields
  document.getElementById('fName').textContent     = fmtOrDash(a.Name);
  document.getElementById('fIndustry').textContent = fmtOrDash(a.Industry);
  document.getElementById('fPhone').textContent    = fmtOrDash(a.Phone);
  ...
  document.getElementById('fOwner').textContent    = fmtOrDash(a.AccountOwner);

  // Case & alert fields — show awaiting if empty
  fmtOrAwaiting(document.getElementById('fCaseId'),       a.Sardine_Case_ID__c);
  fmtOrAwaiting(document.getElementById('fCaseName'),     a.Sardine_Case_Name__c);
  fmtOrAwaiting(document.getElementById('fCaseStatus'),   a.Sardine_Case_Status__c);
  fmtOrAwaiting(document.getElementById('fCaseDecision'), a.Sardine_Case_Decision__c);
  fmtOrAwaiting(document.getElementById('fAlertId'),      a.Sardine_Alert_ID__c);
  fmtOrAwaiting(document.getElementById('fQueue'),        a.Sardine_Queue_Name__c);

  // Risk hero
  const badge = document.getElementById('riskBadge');
  badge.textContent = fmtOrDash(a.Sardine_Risk_Level__c);
  badge.className = 'risk-badge '+(rk||'');
  document.getElementById('riskHero').className = 'risk-hero '+(rk||'');
  document.getElementById('alertStatusVal').textContent = fmtOrDash(a.Sardine_Alert_Status__c);

  // AML / sanctions
  const amlM=a.Sardine_AML_Match_Score__c, amlR=a.Sardine_AML_Risk_Score__c;
  ...
  document.getElementById('sigSanction').innerHTML = hitHtml(a.Sardine_Sanction_Hit__c);
  document.getElementById('sigPep').innerHTML      = hitHtml(a.Sardine_PEP_Hit__c);
  document.getElementById('sigAdverse').innerHTML  = hitHtml(a.Sardine_Adverse_Media_Hit__c);

  // Doc verification
  document.getElementById('sigDocStatus').textContent  = fmtOrDash(a.Sardine_Doc_Status__c);
  document.getElementById('sigDocType').textContent    = fmtOrDash(a.Sardine_Doc_Type__c);
  document.getElementById('sigDocRisk').textContent    = fmtOrDash(a.Sardine_Doc_Risk_Level__c);
  document.getElementById('sigForgery').textContent    = fmtOrDash(a.Sardine_Doc_Forgery__c);
  document.getElementById('sigFaceMatch').textContent  = fmtOrDash(a.Sardine_Doc_Face_Match__c);

  // Device & behavior
  document.getElementById('sigDevRisk').textContent = fmtOrDash(a.Sardine_Device_Risk__c);
  document.getElementById('sigDevSig').textContent  = fmtOrDash(a.Sardine_Device_Signals__c);
  document.getElementById('sigBehav').textContent   = fmtOrDash(a.Sardine_Behavior_Risk__c);

  // Sync ticker
  if(a.Sardine_Last_Updated__c) lastSyncedAt = new Date(a.Sardine_Last_Updated__c).getTime();
...
function renderDemoBtns(sardineId) {
  document.getElementById('demoBtns').innerHTML = sardineId ? \`
    <button class="demo-btn danger"  onclick="trigger('\${sardineId}','escalate')">🚨 very_high risk</button>
    <button class="demo-btn danger"  onclick="trigger('\${sardineId}','aml_flag')">⛔ sanctions hit</button>
    <button class="demo-btn warning" onclick="trigger('\${sardineId}','kyc_fail')">⚠️ kyc_fail</button>
...
async function pollEvents() {
  try {
    ...
    if(newEvs.length){ connEvents=events; renderConnLog(); renderSardineEvents(); }
  } catch(e){}
}

function renderConnLog() {
  document.getElementById('connLogBody').innerHTML = connEvents.slice(0,15).map(e=>{
...
function renderSardineEvents() {
  const recent = connEvents.filter(e=>e.type==='webhook_received').slice(0,8);
  document.getElementById('sardineEvents').innerHTML = recent.length
    ? recent.map(e=>{
        const meta=EVENT_META[e.eventType]||{tag:'alert',icon:'🔔'};
        return \`<div class="ev-item">
          <span class="ev-icon">\${meta.icon}</span>
          <span class="ev-time">\${fmtTime(e.timestamp)}</span>
          <span class="ev-desc">\${e.eventType||'event'}</span>
          <span class="ev-tag \${meta.tag}">\${meta.tag}</span>
        </div>\`;
      }).join('')
    : '<div class="ev-item"><div class="ev-desc awaiting">⏳ Awaiting events…</div></div>';
  // Also mirror into connEvents panel inside sardine panel
  document.getElementById('connEvents').innerHTML = document.getElementById('sardineEvents').innerHTML;
}
...
async function poll() {
  try {
    ...
    document.getElementById('lastRefresh').textContent = fmtTime(new Date().toISOString());
    if(!selectedId && accounts.length) selectedId=accounts[0].Id;
    renderTabs(); renderAccount();
  } catch(e){ document.getElementById('lastRefresh').textContent='error'; }
}
